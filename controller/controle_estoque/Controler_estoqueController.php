...
class Controler_estoqueController 
{
   ...
   public static function Alert_controll()
   {
       try 
       {
           $datas = Controler_estoque::get_quant_min_max(); // pega todos produtos com quantidades
           ...
           if($datas > 0)
           {
               ...
               {
                   if($data->quantidade_max <= $data->quantidade_min) // alerta se quantidade menor ou igual ao mínimo
                   {
                       $alerta_estoque = 1;
               ...
               if($alerta_estoque == 0) // sem alerta
               {
                   session_write_close();
   ...
   public function Entrada_estoque($id, $user_id)
   {
       try 
       {
           $validacao = ValidationProduto::validation_entrada($this->quantidade_max); // valida quantidade
   
           if(!$validacao)
           {
               $quantidade_max = ProdutoController::detalhes($id); // detalhes do produto
               ...
               $this->quantidade_max += $quantidade_max->quantidade_max; // soma quantidade
   
               $entrada = Controler_estoque::update_date_estoque( // atualiza no BD
                   $id,
                   $this->produto_name,
                   $this->preco,
                   $this->quantidade_max,
               ...
               if($entrada)
               {
                   ...
                   MovimentacaoController::entrada($this->produto_name, $quantidade_entrada, $id, $user_id);
                   header("Location: /controler_de_estoque/view/Produtos/index.php");
                   die;  
               }
               else 
   ...
   public function Saida_estoque($id, $user_id)
   {
       try 
       {
           ...
           if(!$validacao)
           {
               ...
               if($this->quantidade_max < 0) $this->quantidade_max = 0; // não permite negativo
   
               $entrada = Controler_estoque::update_date_estoque( // atualiza BD
                   $id,
                   $this->produto_name,
                   $this->preco,
                   $this->quantidade_max,
               ...
               if($entrada)
               {
                   ...
                   MovimentacaoController::saida($this->produto_name, $quantidade_saida, $id, $user_id);
                   header("Location: /controler_de_estoque/view/Produtos/index.php");
                   die; 
               }
               else 
   ...
   // Feedback específico de alertas de estoque
   public static function feedback_systm_alert(string $name_session, string $messagem): void
   {
       $_SESSION[$name_session][] = $messagem; // adiciona mensagem de alerta
   }
